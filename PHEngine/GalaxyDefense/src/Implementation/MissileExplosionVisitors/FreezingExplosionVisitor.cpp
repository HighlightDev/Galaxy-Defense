    ...
    void FreezingExplosionVisitor::ProcessProjectileWithSpaceshipsCollision(const glm::vec3 &explosionCenterPosition)
    {
        if (const auto &ownerSp = mOwnerWp.lock())
        {
            if (const auto &sceneSp = ownerSp->GetSceneOwner().lock())
            {
                ...
                const auto &freezeMissilePhysComponents = combatActorsPoolHandlerSp->GetMissilePhysicsComponents(eMissileType::FREEZING);
                const auto &blackHoleMissilePhysComponents = combatActorsPoolHandlerSp->GetMissilePhysicsComponents(eMissileType::BLACK_HOLE);

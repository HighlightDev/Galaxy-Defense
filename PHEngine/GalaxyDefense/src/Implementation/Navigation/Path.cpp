...
namespace Game {
void Path::AppendPathSegmentToTheEnd(const PathSegment& pathSegment)
{
    mPathSegments.push_back(pathSegment);
}

const std::vector<PathSegment>& Path::GetPathSegments() const
{
...
{
    const auto actualPathSegmentsPointsCount
        = std::accumulate(mPathSegments.cbegin(), mPathSegments.cend(), 0, [](const int32_t total, const PathSegment& segment) {

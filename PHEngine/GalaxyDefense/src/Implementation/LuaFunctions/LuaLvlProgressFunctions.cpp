...
LuaLvlProgressFunctions::LuaLvlProgressFunctions(
    LuaScriptExecutorBase* ownerPtr, const std::shared_ptr<LevelProgressController>& lvlProgressController)
    : mOwnerPtr(ownerPtr)
...
{
    LuaCallbackBindingHelper<Hash64_CT("LuaLvlProgressFunctions::SetLevelProgressStagesQueue"), void(std::string)>::Bind(
        luaWrapper,
        mOwnerPtr,
        std::bind(&LuaLvlProgressFunctions::SetLevelProgressStagesQueue, this, std::placeholders::_1),
        "_SetLevelProgressStagesQueue");
    LuaCallbackBindingHelper<Hash64_CT("LuaLvlProgressFunctions::GetCurrentProgressRequirementsCount"), int32_t()>::Bind(
        luaWrapper,
        mOwnerPtr,
        std::bind(&LuaLvlProgressFunctions::GetCurrentProgressRequirementsCount, this, std::placeholders::_1),
        "_GetCurrentProgressRequirementsCount");
    LuaCallbackBindingHelper<Hash64_CT("LuaLvlProgressFunctions::GetCurrentProgressStageName"), std::string()>::Bind(
        luaWrapper,
        mOwnerPtr,
        std::bind(&LuaLvlProgressFunctions::GetCurrentProgressStageName, this, std::placeholders::_1),
        "_GetCurrentProgressStageName");
    LuaCallbackBindingHelper<Hash64_CT("LuaLvlProgressFunctions::GetCurrentProgressStageRequirementTrackers"), std::string()>::
        Bind(
            luaWrapper,
            mOwnerPtr,
            std::bind(&LuaLvlProgressFunctions::GetCurrentProgressStageRequirementTrackers, this, std::placeholders::_1),
            "_GetCurrentProgressStageRequirementTrackers");
}

void LuaLvlProgressFunctions::SetLevelProgressStagesQueue(
    const std::tuple<std::string /*level progress stages queue json*/>& data)
{

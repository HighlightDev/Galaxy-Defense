...
namespace Game
{
    ...
    std::shared_ptr<SpaceObjectActor>
    AsteroidFactory::CreateSpaceObject(const std::shared_ptr<::EngineCore::Scene> &scene,
                                       const glm::vec3 &translation,
                                       const glm::vec3 &rotation,
                                       const glm::vec3 &scale)
    {
        const auto &asteroidIndexStr = std::to_string(s_asteroidCounter++);
        const auto &rootComponent = std::make_shared<EngineCore::SceneComponent>("c_asteroid_root_" + asteroidIndexStr,

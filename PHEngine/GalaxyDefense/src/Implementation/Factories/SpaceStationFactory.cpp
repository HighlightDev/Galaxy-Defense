...
namespace Game
{
    std::shared_ptr<SpaceStationActor> SpaceStationFactory::CreateSpaceStation(const std::shared_ptr<Scene> &scene,
                                                                               const std::string &towerName,
                                                                               const glm::vec3 &translation,
                                                                               const glm::vec3 &rotation,
                                                                               const glm::vec3 &scale) const
    {
        const auto &towerActor = std::make_shared<SpaceStationActor>(towerName,
                                                                     std::make_shared<SceneComponent>("c_root_" + towerName,
                                                                                                      translation,
                                                                                                      glm::vec3(),
                                                                                                      glm::vec3(1)));
        ...
        const auto &ghostController = std::make_shared<GhostController>(scene->GetPhysicsWorld(), std::make_shared<CollisionSphereShape>(glm::length(scale) * 0.5f), 0.0f);

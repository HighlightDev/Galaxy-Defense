    ...
    const int32_t textFontSize)
{
    ...
    scene->AddActorController(std::make_shared<AiSpaceshipActorController>(a_fighterSpaceship));
    ...
    const auto data = std::make_shared<MeshComponentData>(
        "c_mesh_engine_fighter_" + fighterShipIndexStr,
        "ufo.obj",
...
FighterSpaceShipFactory::GetMaterial(const std::shared_ptr<Scene>& scene, bool& alreadyExists) const
{
    MaterialParser materialParser;

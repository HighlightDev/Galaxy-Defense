...
namespace Game
{
	...
	void BarriersHandler::SelectNewBarrier(const std::string &barrierName)
	{
		...
		const auto &rootComponent = std::make_shared<SceneComponent>("c_barrier_root_" + barrierIndexStr, glm::vec3(), glm::vec3(), glm::vec3(1.0));
	...
	void BarriersHandler::CreateNewBarrierPillar(const glm::vec3 &position,
												 const glm::vec3 &scale)
	{
		...
		const auto d_mesh = std::make_shared<MeshComponentData>("c_barrier_mesh_barrier_" + barrierIndexStr + "_pillar_" + std::to_string(pillarsSize),
																"ufo.obj", position, glm::vec3(), glm::vec3(3.0, 12.0, 3.0), "", barrier_mat.first);
		...
		if (pillarsSize)
		{
			...
			const auto d_mesh = std::make_shared<RuntimeGeneratedMeshComponentData>("c_barrier_mesh_line_" + barrierIndexStr + "_ray_" + std::to_string(rayIndex),
																					4, glm::vec3(0), glm::vec3(), glm::vec3(1), "", barrier_mat.second);
	...
	{
		if ("" != mCurrentActiveBarrierName &&
			mBarrierActors.count(mCurrentActiveBarrierName))
		{
	...
	std::unordered_map<std::string, std::vector<glm::vec3>> BarriersHandler::CollectBarrierPoints() const
	{
		if (mBarrierActors.size())
		{
			...
			for (const auto &[barrierName, barrier] : mBarrierActors)
			{
				...
				std::transform(pillarsMeshVector.cbegin(), pillarsMeshVector.cend(), std::back_inserter(barrierPillarsPositions), [](const auto& pillarMeshComp) {
					return pillarMeshComp->GetTranslation();
				});

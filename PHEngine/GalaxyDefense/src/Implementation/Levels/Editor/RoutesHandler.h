...
class RoutesHandler {
    ...
    std::unordered_map<std::string, std::stack<std::shared_ptr<::EngineCore::RuntimeGeneratedQuadraticBezierCurveComponent>>>
        mActiveRoutes;
...
public:
    explicit RoutesHandler(
        const std::weak_ptr<::EngineCore::Scene>& sceneWp, const std::shared_ptr<::EngineCore::Actor>& bezierCurvesActor);
...
private:
    void SpawnBezierCurveComponent(
        const std::shared_ptr<::Graphics::IMaterial>& splineMaterialPrefab,

...
namespace Game
{
    RoutesHandler::RoutesHandler(const std::weak_ptr<Scene> &sceneWp,
                                 const std::shared_ptr<Actor> &bezierCurvesActor)
        : mSceneWp(sceneWp),
    ...
    void RoutesHandler::SpawnBezierCurveComponent(const std::shared_ptr<IMaterial> &splineMaterialPrefab,
    ...
    void RoutesHandler::AddPointToActiveRoute(const std::shared_ptr<IMaterial> &splineMaterialPrefab, const glm::vec3 &newPoint)
    {
        ...
        const bool bNewPointNotConnectedWithPrevious = (!mActiveRoutes.size() ||
                                                        !mActiveRoutes.count(mCurrentActiveRouteName) ||
                                                        mPotentialBezierCurveControlPoints.size());
        ...
        {
            SpawnBezierCurveComponent(splineMaterialPrefab,
    ...
    std::unordered_map<std::string, std::vector<std::tuple<glm::vec3, glm::vec3, glm::vec3>>> RoutesHandler::CollectRoutesControlPoints() const
    {
        if (mActiveRoutes.size())
        {
            ...
            for (const auto &[routeName, route] : mActiveRoutes)
            {
                ...
                while (!curveComponentsCopy.empty())
                {
                    ...
                    routePathControlPoints.emplace_back(std::make_tuple<glm::vec3, glm::vec3, glm::vec3>(topPathComponent->GetLineBeginWorldSpacePosition(),

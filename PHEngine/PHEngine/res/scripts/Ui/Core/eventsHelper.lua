...
function EventsHelper:sendPauseGameThreadEvent(host, enqueueJobPolicy, isPause)
    assert(enqueueJobPolicy >= 0 or enqueueJobPolicy <= 2,
           "enqueueJobPolicy value must be [0; 2]")
    assert(isPause ~= nil)
...
function EventsHelper:sendLoadLevelGameThreadEvent(host, enqueueJobPolicy,
                                                   levelName)
    assert(enqueueJobPolicy >= 0 or enqueueJobPolicy <= 2,
           "enqueueJobPolicy value must be [0; 2]")
...
function EventsHelper:sendChangeGameModeGameThreadEvent(host, enqueueJobPolicy,
                                                        gameModeType)
    assert(enqueueJobPolicy >= 0 or enqueueJobPolicy <= 2,
           "enqueueJobPolicy value must be [0; 2]")
...
function EventsHelper:sendChangeEditModeGameThreadEvent(host, enqueueJobPolicy,
                                                        editModeType)
    assert(enqueueJobPolicy >= 0 or enqueueJobPolicy <= 2,
           "enqueueJobPolicy value must be [0; 2]")
...
function EventsHelper:sendBroadcastGameThreadEvent(host, enqueueJobPolicy,
                                                   eventHeader, jsonParameters)
    assert(enqueueJobPolicy >= 0 or enqueueJobPolicy <= 2,
           "enqueueJobPolicy value must be [0; 2]")
    assert(eventHeader ~= nil and type(eventHeader) == "string")
    assert(jsonParameters ~= nil and type(jsonParameters) == "string")
    _SendBroadcastGameThreadEvent(host, enqueueJobPolicy, eventHeader,
                                  jsonParameters)
end

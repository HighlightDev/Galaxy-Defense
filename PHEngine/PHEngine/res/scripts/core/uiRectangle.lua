    ...
    if self.luaProxyReady then
        ...
        if replicatorJsonData ~= "" then
            ...
            if parsedJson["color"] ~= nil then
                ...
                self.rectangleProperties.color.value.a = colorArray[4]
            end
...
function UiRectangle:setColorHexValue(colorHex)
    ...
    local mask_a = 0xFF;
    local mask_b = 0xFF << 0x8;
    local mask_g = 0xFF << 0x10;
    local mask_r = 0xFF << 0x18;

    local r = (mask_r & colorHex) >> 0x18;
    local g = (mask_g & colorHex) >> 0x10;
    local b = (mask_b & colorHex) >> 0x8;
    local a = mask_a & colorHex;

...
float voronoi(vec2 p) {
  ...
  for (int y = -1; y <= 1; ++y) {
    for (int x = -1; x <= 1; ++x) {
      ...
      distanceFromPointToCloestFeaturePoint =
          min(distanceFromPointToCloestFeaturePoint, h);
    }
  }
...
float GetMaterialAlphaMask(in MATERIAL_VS_OUTPUT materialIn) { return alpha; };

vec3 GetMaterialWorldNormal(in MATERIAL_VS_OUTPUT materialIn) {
  return vec3(0);
}

vec4 GetMaterialEmission(in MATERIAL_VS_OUTPUT materialIn) { return vec4(0.0); }
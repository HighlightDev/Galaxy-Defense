...
vec3 GetMaterialAlbedo(in MATERIAL_VS_OUTPUT materialIn)
{
    ...
    vec3 mixedIceColor
        = mix(albedoColor, albedo_ice_color, smoothstep(0.0, 0.5, GetFreezingEffectForCurrentInstance(materialIn)));
...
vec2 GetMaterialMetallicRoughness(in MATERIAL_VS_OUTPUT materialIn)
{
    ...
    vec2 mixedIceMetallicRoughness = mix(
        ...
        smoothstep(0.0, 0.5, GetFreezingEffectForCurrentInstance(materialIn)));
...
vec3 GetMaterialWorldNormal(in MATERIAL_VS_OUTPUT materialIn)
{
    ...
    vec3 mixedIceNormalTangentSpace
        = mix(normal, normal_ice, smoothstep(0.0, 0.5, GetFreezingEffectForCurrentInstance(materialIn)));

...
vec3 getSpaceStarsColor(in MATERIAL_VS_OUTPUT materialIn) {
  ...
  for (int r = 0; r < volsteps; r++) {
    ...
    p_varying =
        abs(vec3(tile) - mod(p_varying, vec3(tile * 2.0))); // tiling fold
    float pa, a = pa = 0.0;
    for (int i = 0; i < iterations; i++) {
      p_varying = abs(p_varying) / dot(p_varying, p_varying) -
                  formuparam; // the magic formula
    ...
    v += fade;
    float s_sqr = s * s;
    v += vec3(s, s_sqr, s_sqr * s_sqr) * a * brightness *

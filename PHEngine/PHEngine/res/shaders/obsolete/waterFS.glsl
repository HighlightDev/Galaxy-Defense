...
vec4 specularPointLight(vec3 specularNormal, vec3 ligthDiffuseV, vec3 InAttenuation, vec3 specularColor, float softEdge)
{
    ...
    float attenuationLVL
        = 1.0 / (InAttenuation.x + (InAttenuation.y * lightDistance) + (InAttenuation.z * (lightDistance * lightDistance)));
...
void main(void)
{
    ...
    float floorDistance = (2.0 * nearClipPlane * farClipPlane)
        / (farClipPlane + nearClipPlane - (2.0 * depth - 1.0) * (farClipPlane - nearClipPlane));

    // depth to water plane
    depth = gl_FragCoord.z;
    float waterDistance = (2.0 * nearClipPlane * farClipPlane)
        / (farClipPlane + nearClipPlane - (2.0 * depth - 1.0) * (farClipPlane - nearClipPlane));
    ...
    vec2 totalDistortion
        = (texture(dudvTexture, distortedTexCoords).rg * 2.0 - 1.0) * waveStrength * clamp(waterDepth / 20.0, 0.0, 1.0);
    ...
    for (int i = 0; i < MAX_LIGHT_COUNT; i++) {
        ...
        pointLightColor
            = pointLightColor
            + specularPointLight(
                  SpecularNormal, toLightDiffuseVec[i], attenuation[i], specularColour[i], clamp(waterDepth / 5.0, 0.0, 1.0));
    }

...
subroutine(LensFlare) vec4 lensSimple(vec2 TC)
{
    vec4 texColor = texture(frameTexture, TC);
    return texColor;
}

subroutine(LensFlare) vec4 lensThreshold(vec2 TC)
{
    vec4 texColor = texture(frameTexture, TC);
...
subroutine(LensFlare) vec4 lensEffect(vec2 TC)
{
...
subroutine(LensFlare) vec4 vertBlur(vec2 TC)
{
...
subroutine(LensFlare) vec4 horizBlur(vec2 TC)
{
...
subroutine(LensFlare) vec4 lensModifer(vec2 TC)
{

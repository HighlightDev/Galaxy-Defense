...
{
    return glm::vec2(
        static_cast<float>(mAbsoluteOrigin.x) / static_cast<float>(mWidthHeight.x),
...
void UiCanvas::UnpausableTick(const float deltaTimeSec, const float playSpeed)
{
    ...
    if (mInputSystem && mIsVisible) {
        mInputSystem->UnpausableTick(deltaTimeSec, playSpeed);
    }
...
void UiCanvas::Tick(const float deltaTimeSec, const float playSpeed)
{
}
...
UiCanvas::GetDependentByTransformChildren(const std::string& nameOfChangedTransformUiItem) const
{
...
void UiCanvas::CollectChildrenWithDescendingZOrder()
{
    ...
    std::transform(
        childrenWithDescendingOrder.begin(),
        childrenWithDescendingOrder.end(),
        std::back_inserter(mDescendingByZOrderHierarchyChildren),
        [](const auto& childSp) { return std::weak_ptr<UiItemBase>(childSp); });
}

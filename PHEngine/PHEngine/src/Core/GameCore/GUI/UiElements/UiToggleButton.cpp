        ...
        if (const auto& sceneRendererSp = sceneSp->GetInterThreadCommunicationManager().GetSceneRendererWP().lock()) {
            if (const auto& parentCanvasSp = mParentCanvas.lock()) {
                sceneRendererSp->RegisterUiSceneProxy_OnRenderThread(
                    std::static_pointer_cast<UiToggleButton>(shared_from_this()), CreateUiSceneProxy(), parentCanvasSp->GetUId());
            }
        }
    }
}
                ...
                if (const auto& sceneRenderer = sceneSp->GetInterThreadCommunicationManager().GetSceneRendererWP().lock()) {
                    ...
                    sceneSp->GetInterThreadCommunicationManager().ExecuteOnRenderThread(
                        ...
                        [sceneRenderer,
                         ...
                         toggleOnColor = mToggleOnColor,
                         toggleOffColor = mToggleOffColor,
                         isStateOn = mIsStateOn,
                         opacity = mOpacity](
                            ...
                            if (uiSceneProxy) {
                                const auto& toggleButtonSceneProxy
                                    = std::static_pointer_cast<UiToggleButtonSceneProxy>(uiSceneProxy);
                ...
                sceneSp->GetInterThreadCommunicationManager().ExecuteOnLuaThread(
                    ...
                    [luaScriptProcessorSp,
                     ...
                     isStateOn = mIsStateOn](
                        ...
                        std::weak_ptr<EngineCore::Scene> sceneWp,
                        std::weak_ptr<::EngineCore::Scripts::LuaScriptProcessor> luaProcessorWp) {
                        if (const auto& toggleButtonLuaProxy
                            = std::static_pointer_cast<UiToggleButtonLuaProxy>(luaScriptProcessorSp->GetLuaProxy(luaProxyId))) {

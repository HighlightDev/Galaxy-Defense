            ...
            if (const auto &sceneSp = GetScene().lock())
            {
                if (const auto &sceneRendererSp = sceneSp->GetInterThreadCommunicationManager().GetSceneRendererWP().lock())
                {
                    if (const auto &parentCanvasSp = mParentCanvas.lock())
                    {
                        ...
                        sceneRendererSp->RegisterUiSceneProxy_OnRenderThread(std::static_pointer_cast<UiProgressBar>(shared_from_this()), thisSceneProxy, parentCanvasSp->GetUId());
                    }
                }
            }
        }
                ...
                if (const auto &sceneSp = GetScene().lock())
                {
                    if (const auto &canvasSp = GetParentCanvas().lock())
                    {
                        if (const auto &sceneRenderer = sceneSp->GetInterThreadCommunicationManager().GetSceneRendererWP().lock())
                        {
                            ...
                            sceneSp->GetInterThreadCommunicationManager().ExecuteOnRenderThread(eEnqueueJobPolicy::IF_DUPLICATE_REPLACE, GetUId(), functionId, [sceneRenderer, myUId = GetUId(), canvasUId = canvasSp->GetUId(), emptyColor = mEmptyColor, filledColor = mFilledColor, opacity = mOpacity, fillPercentValue = mFillPercentValue]()
                                                                                                {
            ...
            if (mIsLuaProxyReady.load(std::memory_order::memory_order_seq_cst))
            {
                if (const auto &sceneSp = GetScene().lock())
                {
                    if (const auto &luaScriptProcessorSp = GetLuaScriptProcessorWp().lock())
                    {
                        ...
                        sceneSp->GetInterThreadCommunicationManager().ExecuteOnLuaThread(eEnqueueJobPolicy::IF_DUPLICATE_REPLACE, GetUId(), functionId, [luaScriptProcessorSp, luaProxyId = GetLuaProxyId(), opacity = mOpacity, emptyColor = mEmptyColor, filledColor = mFilledColor, fillPercentValue = mFillPercentValue]()
                                                                                         {
                                                                                             if (const auto &rectangleLuaProxy = std::static_pointer_cast<UiProgressBarLuaProxy>(luaScriptProcessorSp->GetLuaProxy(luaProxyId)))
                                                                                             {

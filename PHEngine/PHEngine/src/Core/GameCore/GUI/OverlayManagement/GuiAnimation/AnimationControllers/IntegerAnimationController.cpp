...
namespace EngineCore
{
    namespace GUI
    {
        void IntegerAnimationController::ProcessAnimation(const float animationTimePassed,
                                                        ...
                                                        const std::shared_ptr<IAnimatable> &animatable)
        {
            ...
            else
            {
                if (eAnimationInterpolationFunctionType::LINEAR == data.GetAnimationFunctionType())
                {
                    ...
                    const int32_t resultValue = EngineMath::LerpInt(animationTimePassed, 0.0f, data.GetAnimationDuration(), srcValue, dstValue);
        ...
        void IntegerAnimationController::ForceFinishAnimation(const AnimationData &data,
                                                            const std::shared_ptr<IAnimatable> &animatable)
        {
            const auto &engineProperty = animatable->GetPropertyByName(data.GetPropertyName());
            assert(engineProperty);
        ...
        void IntegerAnimationController::InitWithSrcValues(const AnimationData &data,
                                                         const std::shared_ptr<IAnimatable> &animatable)
        {
            const auto &engineProperty = animatable->GetPropertyByName(data.GetPropertyName());
            assert(engineProperty);

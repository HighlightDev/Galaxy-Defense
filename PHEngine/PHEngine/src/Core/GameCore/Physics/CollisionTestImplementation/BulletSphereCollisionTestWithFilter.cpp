...
namespace EnginePhysics
{
    ...
    BulletSphereCollisionTestWithFilter::BulletSphereCollisionTestWithFilter(const float sphereRadius, std::vector<btCollisionObject *> excludeCollisionObjects)
        : btCollisionWorld::ContactResultCallback(),
          mSphereShape(std::make_unique<btSphereShape>(sphereRadius)),
          mGhostSphereObject(),
    ...
    btScalar BulletSphereCollisionTestWithFilter::addSingleResult(btManifoldPoint &cp, const btCollisionObjectWrapper *colObj0,
                                                                  ...
                                                                  int index1)
    {
        ...
        if (mGhostSphereObject.get() != collidedObject && collidedObject->getUserPointer())
        {
            if (const auto &collidedObjDescriptor = reinterpret_cast<const PhysicsDescriptor *>(collidedObject->getUserPointer()))
            {
                ...
                {
                    const auto isExludedBody = std::any_of(mExcludeFilterBodies.cbegin(), mExcludeFilterBodies.cend(),
                                                           [collidedObject](const auto &excludeCollObj)

...
{
    return std::any_of(
               mExcludeFilterBodies.cbegin(),
               mExcludeFilterBodies.cend(),
               [&rayResult](const auto& excludeCollObj) { return rayResult.m_collisionObject == excludeCollObj; })
        ? 1.0f
        : ClosestRayResultCallback::addSingleResult(rayResult, normalInWorldSpace);
}

void BulletRayCastWithFilter::RayTest(
    const btDiscreteDynamicsWorld* physWorld, const btVector3& rayFromPosition, const btVector3& rayToPosition)
{

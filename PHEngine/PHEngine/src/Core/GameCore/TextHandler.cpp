...
namespace EngineCore
{
    ...
    std::shared_ptr<HudTextField> TextHandler::CreateTextField(const std::string &fontName,
    ...
    {
        const auto &textField = mRegisteredTexts.emplace_back(std::make_shared<HudTextField>(fontName,
                                                                                             fontSize,
                                                                                             text,
                                                                                             color,
                                                                                             position,
    ...
    std::shared_ptr<HudTextField> TextHandler::CreateEmptyTextField(const std::string &fontName,
                                                                    const float fontSize,
                                                                    const glm::vec3 &color,
                                                                    const bool receiveUpdateOnTextScreenSpaceSizeChanged,
                                                                    const float lineMaxSize,
    ...
    void TextHandler::RegisterTextField(const std::shared_ptr<HudTextField> &textField,
                                        const bool receiveUpdateOnTextScreenSpaceSizeChanged)
    {
        if (const auto &sceneSp = mSceneWp.lock())
        {

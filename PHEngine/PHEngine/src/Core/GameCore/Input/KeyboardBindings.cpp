...
void KeyboardBindings::ProcessEvent(
    const KeyboardButtonDownGameThreadEvent* sender, const KeyboardButtonDownGameThreadEvent::EventData_t& eventData)
{
...
KeyState KeyboardBindings::GetKeyStateByActionType(eKeyActionType actionType) const
{
    ...
    auto it = std::find_if(
        mKeyboardMaskVec.begin(), mKeyboardMaskVec.end(), [=](const auto& keyData) -> bool { return keyData.Key == key; });

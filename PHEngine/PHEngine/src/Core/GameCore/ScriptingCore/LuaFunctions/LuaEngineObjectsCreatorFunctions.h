...
class LuaEngineObjectsCreatorFunctions : public ILuaFunctionable {
...
private:
    ...
    /* -------------------  Create Actor ----------------------------*/
    int32_t CreateActor(const std::tuple<
    ...
    void CreateFirstPersonCamera(
        const std::tuple<std::string, glm::ivec4, std::string /*viewProjectionJsonArgs*/, float, float, glm::vec3, int32_t>&
            cameraData);
    ...
    void SetTextureToMaterial(
        const std::tuple<int32_t /*material proxy id*/, std::string /* texture name*/, std::string /*property name*/>&
            setTextureToMaterial);
    ...
    void SetDeferredTextureToMaterial(
        const std::
            tuple<int32_t /*material proxy id*/, /*deferred resource creator name*/ std::string, /*property name*/ std::string>&
                setDeferredTextureToMaterial);

    /* -------------------  Set binding to material --------------------*/
    void SetBindingToMaterial(const std::tuple<
                              int32_t /*material proxy id*/,
                              /*game object name*/ std::string,
    ...
    void SetTweenerBinding(const std::tuple<

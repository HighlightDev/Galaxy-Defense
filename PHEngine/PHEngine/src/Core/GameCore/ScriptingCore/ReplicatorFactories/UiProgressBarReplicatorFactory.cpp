...
namespace EngineCore
{
    namespace Scripts
    {
        int32_t UiProgressBarReplicatorFactory::CreateReplicator(const std::weak_ptr<Scene> &sceneWp,
                                                                 ...
                                                                 const std::string &jsonParamsStr) const
        {
            ...
            if (const auto &sceneSp = sceneWp.lock())
            {
                ...
                sceneSp->GetInterThreadCommunicationManager().ExecuteOnGameThread(eEnqueueJobPolicy::IF_DUPLICATE_NO_PUSH, uiProgressBarLuaProxyId, functionId, [sceneSp, luaScriptProcessorWp, uiProgressBarLuaProxyId, name]()
                                                                                  {

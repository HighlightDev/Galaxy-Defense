...
void Level::CollectAllocatedResourcesForSerialization(SerializeDataContainer& container)
{
    ...
    std::transform(
        meshParamsList.cbegin(),
        meshParamsList.cend(),
        std::back_inserter(loadedModelNames),
        [](const MeshPoolParameters& meshParam) { return meshParam.mModelPath; });

    std::vector<std::string> concatNamesVec;
    concatNamesVec.reserve(loadedTextureNames.size() + loadedModelNames.size());
    concatNamesVec.insert(
        concatNamesVec.end(),
        std::make_move_iterator(loadedTextureNames.begin()),
        std::make_move_iterator(loadedTextureNames.end()));

    concatNamesVec.insert(
        concatNamesVec.end(), std::make_move_iterator(loadedModelNames.begin()), std::make_move_iterator(loadedModelNames.end()));
        ...
        for (const auto& actorData : container.Actors) {
            if (actorData.TweenerData) {
                ...
                auto actorIt = std::find_if(
                    sceneSp->GetActors().begin(), sceneSp->GetActors().end(), [&](const std::shared_ptr<Actor> actor) {

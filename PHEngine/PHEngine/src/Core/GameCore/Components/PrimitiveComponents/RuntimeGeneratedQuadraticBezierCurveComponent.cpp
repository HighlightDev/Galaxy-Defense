...
namespace EngineCore
{
   ...
   void RuntimeGeneratedQuadraticBezierCurveComponent::SyncRenderData()
   {
      ...
      if (const auto &sceneSp = m_sceneWP.lock())
      {
         ...
         {
            sceneSp->GetInterThreadCommunicationManager().ExecuteOnRenderThread(eEnqueueJobPolicy::IF_DUPLICATE_REPLACE, GetObjectId(), functionId, [sceneRenderer, sceneProxyId = mSceneProxyId, bezierControlPosition = mBezierControlPointWorldSpacePosition, curveSegmentsCount = mCurveSegmentsCount, lineBeginWorldSpacePosition = GetLineBeginWorldSpacePosition(), lineEndWorldSpacePosition = GetLineEndWorldSpacePosition(), lineWidth = GetLineWidth()]()
                                                                                {
            if (const auto &lineProxySp = std::static_pointer_cast<RuntimeGeneratedQuadraticBezierCurveSceneProxy>(sceneRenderer->GetPrimitiveProxyByProxyId(sceneProxyId)))
            {

...
void LightComponent::UnpausableTick(const float deltaTimeSec, const float playSpeed)
{
        ...
        if (const auto& sceneSP = m_sceneWP.lock()) {
            if (const auto& sceneRendererSp = sceneSP->GetInterThreadCommunicationManager().GetSceneRendererWP().lock()) {
                if (bIsEnabledStateDirty) {
                    ...
                    sceneRendererSp->UpdateLightComponentEnable_OnRenderThread(
                        mLightSceneProxyId, GetObjectId(), functionId, mIsEnabled->GetValue());
                ...
                if (bIsVisibleStateDirty) {
                    ...
                    sceneRendererSp->UpdateLightComponentIsVisible_OnRenderThread(
                        mLightSceneProxyId, GetObjectId(), functionId, mIsVisible);

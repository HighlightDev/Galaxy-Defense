...
struct DynamicMaterialProperty {
    enum class eDynamicMaterialPropertyType { FloatProperty, iVec2Property, Vec2Property, Vec3Property, InstancedFloatProperty };
...
public:
    ...
    std::shared_ptr<MaterialProperty> TryGetInternalMaterialPropertyByName(const std::string& propertyName) const
    {
        auto propertyIt = std::find_if(
            mInternalDynamicMaterialProperties.begin(),
            mInternalDynamicMaterialProperties.end(),

...
void CameraFrustum::ConstructFromViewProjectionMatrix(const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix)
{
    ...
    // left clipping plane
    for (int i = 4; i--;)
        (mPlanes[0])[i] = comboMatrix[i][3] + comboMatrix[i][0];
    // right clipping plane
    for (int i = 4; i--;)
        (mPlanes[1])[i] = comboMatrix[i][3] - comboMatrix[i][0];
    // bottom clipping plane
    for (int i = 4; i--;)
        (mPlanes[2])[i] = comboMatrix[i][3] + comboMatrix[i][1];
    // top clipping plane
    for (int i = 4; i--;)
        (mPlanes[3])[i] = comboMatrix[i][3] - comboMatrix[i][1];
    // near clipping plane
    for (int i = 4; i--;)
        (mPlanes[4])[i] = comboMatrix[i][3] + comboMatrix[i][2];
    // far clipping plane
    for (int i = 4; i--;)
        (mPlanes[5])[i] = comboMatrix[i][3] - comboMatrix[i][2];
...
CameraFrustum
CameraFrustum::GetConstructedFromViewProjectionMatrices(const glm::mat4& viewMatrix, const glm::mat4& projectionMatrix)
{
...
glm::vec4 CameraFrustum::GetPlaneByName(const eFrustumPlaneName planeName) const
{
    return mPlanes[(uint8_t)(planeName)];
}
} // namespace Graphics
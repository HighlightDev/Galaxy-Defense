...
std::string FramebufferObject::GetFramebufferLog() const
{
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_DRAW_BUFFER: {
        return "Framebuffer : Each draw buffer must specify color attachment points that have images attached or must be "
               "GL_NONE.";
    }
    ...
    case GL_FRAMEBUFFER_UNSUPPORTED: {
        return "Framebuffer : The combination of internal formats of the attached images violates an implementation-dependent "
               "set of restrictions.";
    }
    default:
...
void FramebufferObject::CollectAttachments()
{
    ...
    if (attachmentsCount) {
        ...
        std::sort(
            mFramebufferAttachments.begin(), mFramebufferAttachments.end(), [](const auto& left, const auto& right) -> bool {
                return left < right;
            });
    }
}
...
void FramebufferObject::CreateRenderBuffer(
    const int32_t renderbufferDataType,
    const int32_t framebufferRenderbufferAttachment,

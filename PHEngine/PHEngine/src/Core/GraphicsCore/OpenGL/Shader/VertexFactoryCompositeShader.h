...
class VertexFactoryCompositeShader : public AVertexFactoryShaderModule {
protected:
    std::shared_ptr<Shader> mBaseShader;

public:
    VertexFactoryCompositeShader(const CompositeShaderParams& compositeShaderParams)
        : AVertexFactoryShaderModule(compositeShaderParams, std::make_shared<VertexFactoryShaderType>())

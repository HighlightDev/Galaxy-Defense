...
struct AnimatedMeshData {
    ...
    std::map<std::string, BoneData> GetBoneMappingForBlendedAnimation(
...
private:
    void GetBlendedBoneDataNodeHierarchy(
        const float srcTime,
        const float dstTime,
        ...
        const float blendFactor,
        MeshNode* node,
        std::map<std::string, BoneData>& blendedBoneData) const;
    ...
    void ReadNodeHierarchyWithBlendedBoneData(
        MeshNode* node,
        const std::map<std::string, BoneData>& blendedBoneData,
        const glm::mat4& parentTransform,
        std::vector<glm::mat4>& finalOutput) const;

...
void CubemapSceneProxy::PostConstructorInitialize()
{
    ...
    cubemapShaderParams.SetMainShaders(
        FolderManager::GetInstance()->GetAbsolutePathToRes("cubemapRendererVS.glsl"),
        FolderManager::GetInstance()->GetAbsolutePathToRes("cubemapRendererFS.glsl"));
        ...
        if (const auto& sceneSp = deferredShadingSceneRendererSp->GetInterThreadCommunicationManager().GetSceneWP().lock()) {
            ...
            sceneSp->GetInterThreadCommunicationManager().ExecuteOnGameThread(
                ...
                [sceneSp, boundingBox, goID = GetGameObjectId()](
                    ...
                    ext_assert(engineObject, "Engine object not found by ID in CubemapSceneProxy");
                    const auto& primitiveComponent = std::static_pointer_cast<PrimitiveComponent>(engineObject);
                    ext_assert(primitiveComponent, "Failed to cast engine object to PrimitiveComponent in CubemapSceneProxy");

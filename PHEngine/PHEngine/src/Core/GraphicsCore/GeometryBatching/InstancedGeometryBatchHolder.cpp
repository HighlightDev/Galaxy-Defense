...
namespace EngineCore
{
    ...
    void InstancedGeometryBatchHolder::Tick(const float deltaTime)
    {
        ...
        if (const auto &sceneSp = mSceneWp.lock())
        {
            ...
            if (const auto &sceneRendererSp = threadMngr.GetSceneRendererWP().lock())
            {
                ...
                threadMngr.ExecuteOnRenderThread(eEnqueueJobPolicy::IF_DUPLICATE_REPLACE, 0, functionId, [sceneRendererSp, data = std::move(batchData)]
                                                 {
    ...
    {
        const auto isNewBatch = std::none_of(mInstancedGeometryBatches.cbegin(), mInstancedGeometryBatches.cend(), [newBatchKey = batch->GetBatchKey()](const auto &batchSp)
                                             { return newBatchKey == batchSp->GetBatchKey(); });
    ...
    {
        const auto bBatchExists = std::any_of(mInstancedGeometryBatches.cbegin(), mInstancedGeometryBatches.cend(), [batchKey](const auto &batchSp)
    ...
    {
        const auto foundIt = std::find_if(mInstancedGeometryBatches.cbegin(), mInstancedGeometryBatches.cend(), [batchKey](const auto &batchSp)
                                          { return batchKey == batchSp->GetBatchKey(); });

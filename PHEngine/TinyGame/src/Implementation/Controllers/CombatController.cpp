...
void CombatController::OnLevelInit()
{
    if (const auto& sceneSp = mScene.lock()) {
        ...
        for (size_t i = 0; i < 2; ++i) {
            const auto& a_enemyShip = spaceShipFactory.CreateSpaceShip(sceneSp, glm::vec3(), glm::vec3(), glm::vec3(9));
...
void CombatController::CreateBackgroundSpaceObjectsPool(const std::shared_ptr<Scene>& sceneSp)
{
    ...
    mBackgroundSpaceObjects.emplace_back(
        factory.CreateSpaceObject(sceneSp, glm::vec3(0.0f, 0.0f, 0.01f), glm::vec3(), glm::vec3(1), "planet_1.png", 15.0f));
    mBackgroundSpaceObjects.emplace_back(
        factory.CreateSpaceObject(sceneSp, glm::vec3(0.0f, 0.0f, 0.02f), glm::vec3(), glm::vec3(1), "planet_2.png", 5.0f));
    mBackgroundSpaceObjects.emplace_back(
        factory.CreateSpaceObject(sceneSp, glm::vec3(0.0f, 0.0f, 0.03f), glm::vec3(), glm::vec3(1), "planet_3.png", 7.0f));
    mBackgroundSpaceObjects.emplace_back(
        factory.CreateSpaceObject(sceneSp, glm::vec3(0.0f, 0.0f, 0.04f), glm::vec3(), glm::vec3(1), "planet_4.png", 10.0f));
    mBackgroundSpaceObjects.emplace_back(
        factory.CreateSpaceObject(sceneSp, glm::vec3(0.0f, 0.0f, 0.05f), glm::vec3(), glm::vec3(1), "planet_5.png", 12.0f));
}
    ...
    if (eMainPlayerActionEnum::SHOOT == playerAction && !bIsCoolDownInProgress) {
        ...
        if (const auto& sceneSp = mScene.lock()) {
            ...
            LuaMainPlayerStatusChangedEvent::GetInstance()->SendEvent(
                eExecutionOrder::POST_EXECUTION, eMainPlayerStatusType::LIFE_POINTS_CHANGED);
        }
    } else if (
        eMainPlayerActionEnum::SELECT_NEXT_MISSILE_TYPE == playerAction
        || eMainPlayerActionEnum::SELECT_PREV_MISSILE_TYPE == playerAction) {
        static std::array<eMissileType, 4> missiles
            = {eMissileType::BOMB, eMissileType::FREEZING, eMissileType::ELECTRO_RAY, eMissileType::BLACK_HOLE};
            ...
            if (objectsCollisionType != eGameObjectsCollisionType::UNDEFINED) {
                if (eGameObjectsCollisionType::SPACESHIP_WITH_MISSILE == objectsCollisionType) {
                    const auto& ownerEnemyShipActor = eGameObjectsType::SPACESHIP == thisActorGameObjectType
                        ? GetEnemyShipOwnerActorById(this_actor_id)
                        : GetEnemyShipOwnerActorById(that_actor_id);
                    const auto& ownerMissileActor = eGameObjectsType::MISSILE == thisActorGameObjectType
                        ? GetMissileOwnerActorById(this_actor_id)
                        : GetMissileOwnerActorById(that_actor_id);
                    const auto& spaceshipActor_id
                        = eGameObjectsType::SPACESHIP == thisActorGameObjectType ? this_actor_id : that_actor_id;
                    const auto& missileActor_id
                        = eGameObjectsType::MISSILE == thisActorGameObjectType ? this_actor_id : that_actor_id;
                    ...
                    LogInfo(
                        "CombatController::PhysicsCollisionEvent =>",
                        collisionType,
                    ...
                    const auto& ownerSpaceObjectActor = eGameObjectsType::NEUTRAL_SPACE_OBJECT == thisActorGameObjectType
                        ? GetSpaceObjectOwnerActorById(this_actor_id)
                        : GetSpaceObjectOwnerActorById(that_actor_id);
...
void CombatController::Tick(const float deltaTime)
{
    ...
    if (mCameraVisibilityArea) {
        for (const auto& backgroundObject : mBackgroundSpaceObjects) {
            if (eSpaceObjectActivityState::ACTIVE == backgroundObject->GetActivityState()
                && !backgroundObject->IsInsideLevel(*mCameraVisibilityArea)) {
                backgroundObject->TriggerDisabled();
            }
        }
    }
...
void CombatController::OnBackgroundPlanetsSpawnTimerTimeout()
{
    if (mCameraVisibilityArea) {
        ...
        if (bShouldSpawnBackground) {
            const float x = mCameraVisibilityArea->GetOrigin().x
                + mCameraVisibilityArea->GetHalfExtent().x * ((2.0f * Random::Float()) - 1.0f);
            const auto foundFree
                = std::find_if(mBackgroundSpaceObjects.begin(), mBackgroundSpaceObjects.end(), [](const auto& object) {
            ...
            if (foundFree != mBackgroundSpaceObjects.end()) {
                ...
                LogInfo(
                    "CombatController::OnBackgroundPlanetsSpawnTimerTimeout => Spawn background planet. Object Id: ",
                    (*foundFree)->GetObjectId());
            }
        }
    }
}
...
glm::vec3 CombatController::GenRandomPositionForSpaceObject() const
{
    ...
    do {
        ...
        return (
            (eSpaceObjectActivityState::ACTIVE == existingSpaceObject->GetActivityState())
            && ((glm::length2(existingSpaceObject->GetWorldPosition() - potentialPosition) < min_squared_radius)));
    }));
...
glm::vec3 CombatController::GenRandomPositionForSpaceship() const
{
    ...
    do {
        ...
        return (
            (eSpaceshipActivityState::ACTIVE == existingEnemy->GetSpaceshipActivityState())
            && ((glm::length2(existingEnemy->GetWorldPosition() - potentialPosition) < min_squared_radius)));
    }));
...
{
    return glm::vec3(
        mCameraVisibilityArea->GetOrigin().x, mCameraVisibilityArea->GetOrigin().y, mCameraVisibilityArea->GetMax().z);
}
...
void CombatController::CreateWeaponBulletPool(const std::shared_ptr<Scene>& sceneSp)
{
    ...
    for (size_t i = 0; i < electroRayCount; ++i) {
        const auto& a_electroRay
            = electroRayFactory.CreateMissile(sceneSp, mPlayerShip, glm::vec3(0), glm::vec3(), glm::vec3(1.0));
    ...
    for (size_t i = 0; i < freezingMissileCount; ++i) {
        const auto& a_missile
            = freezingMissileFactory.CreateMissile(sceneSp, mPlayerShip, glm::vec3(0), glm::vec3(), glm::vec3(1.0));
    ...
    for (size_t i = 0; i < bombMissileCount; ++i) {
        const auto& a_missile = bombMissileFactory.CreateMissile(sceneSp, mPlayerShip, glm::vec3(0), glm::vec3(), glm::vec3(1.0));
    ...
    for (size_t i = 0; i < blackHoleMissileCount; ++i) {
        const auto& a_missile
            = blackHoleMissileFactory.CreateMissile(sceneSp, mPlayerShip, glm::vec3(0), glm::vec3(), glm::vec3(1.0));
...
void CombatController::CreateAsteroidsPool(const std::shared_ptr<Scene>& sceneSp)
{
    ...
    for (size_t i = 0; i < 5; ++i) {
        const auto& a_asteroid = asteroidFactory.CreateSpaceObject(sceneSp, glm::vec3(0), glm::vec3(), glm::vec3(40.0));
...
void CombatController::ShootBullet(const glm::vec3& bulletStartPosition)
{
    ...
    auto idleBulletIt = std::find_if(mMissilesPool.begin(), mMissilesPool.end(), [selectedMissileType](const auto& missile) {
        return (
            ...
            && selectedMissileType == missile->GetMissileType());
    });
...
eGameObjectsType CombatController::GetGameObjectTypeByActorId(const int32_t actorId) const
{
    ...
    result = std::any_of(
                 mEnemies.begin(),
                 mEnemies.end(),
                 [actorId](const auto& spaceshipActor) { return spaceshipActor->HasEngineObjectIdInHierarchy(actorId); })
    ...
    if (eGameObjectsType::UNDEFINED == result) {
        result = std::any_of(
                     mMissilesPool.begin(),
                     mMissilesPool.end(),
                     [actorId](const auto& missileActor) { return missileActor->HasEngineObjectIdInHierarchy(actorId); })
    ...
    if (eGameObjectsType::UNDEFINED == result) {
        result = std::any_of(
                     mSpaceObjectsPool.begin(),
                     mSpaceObjectsPool.end(),
                     [actorId](const auto& spaceObjectActor) { return spaceObjectActor->HasEngineObjectIdInHierarchy(actorId); })
...
CombatController::GetGameObjectsCollisionType(const eGameObjectsType firstObject, const eGameObjectsType secondObject) const
{

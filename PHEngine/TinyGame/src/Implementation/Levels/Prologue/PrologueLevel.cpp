    ...
    : LevelBase("FirstLevel")
{
    Event::GameThreadEventDispatcher::GetInstance()
        ->RegisterEventsByType<
            Event::MainPlayerActionEvent,
            Event::RayCollisionEvent,
            Event::SphereContactCollisionEvent,
...
void PrologueLevel::CreateScene()
{
    ...
    auto spaceCamera = std::make_shared<SpaceSceneCamera>(
        "SpaceShipCamera",
    ...
    const auto& c_input
        = sceneSp->CreateComponent_GameThread(inputComponentCreator, std::make_shared<ComponentData>("SpaceshipInputComponent"));
    ...
    const auto& c_movement = std::static_pointer_cast<NoPhysicsMovementComponent>(sceneSp->CreateComponent_GameThread(
        movementComponentCreator, std::make_shared<MovementComponentData>("NoPhysMoveComponentData", glm::vec3())));
    ...
    assert(mainCamera);
    const std::shared_ptr<SpaceShipPlayerController>& spaceShipController
        = std::make_shared<SpaceShipPlayerController>(mainCamera, a_spaceship);
    ...
    plShadowTexAtlasRequest); const auto cubemapRendererComponent = sceneSp->CreateComponent_GameThread<CubemapComponent,
    EngineCore::eComponentMetaType::Cubemap>(cubemapComponentData); groundActor->AddComponent(cubemapRendererComponent);*/
